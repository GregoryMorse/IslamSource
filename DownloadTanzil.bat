...
FOR /F delims^=^"^ tokens^=6^,12 %%I IN ('FIND "http://tanzil.net/trans/" index.html') DO IF "%%J"=="return download(this)" (FOR /F "delims=^/ tokens=4" %%Q IN ("ECHO %%I") DO (e:\wsusoffline\bin\wget.exe -S -N --content-disposition "http://tanzil.net/trans/log/%%Q")) ELSE (FOR /F "delims=^/ tokens=4" %%Q IN ("ECHO %%J") DO (e:\wsusoffline\bin\wget.exe -S -N --content-disposition "http://tanzil.net/trans/log/%%Q"))
REM find /C * "2014"
