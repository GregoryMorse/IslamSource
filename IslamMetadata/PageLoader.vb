...
Public Class Arabic
    ...
    Public Shared ColoringSpelledOutRules As RuleTranslation() = { _
        ...
        Public Shared RulesOfRecitationRegEx As RuleMetadataTranslation() = { _
            ...
            New RuleMetadataTranslation With {.Rule = "VowellessMeemClearGreater", .Match = "(?:(" + MakeUniRegEx(ArabicLetterMeem) + MakeUniRegEx(ArabicSukun) + ")|(" + MakeUniRegEx(ArabicLetterMeem) + ")(?:\s+))(?=(" + MakeUniRegEx(ArabicLetterFeh) + "|((" + MakeUniRegEx(ArabicSukun) + "|" + MakeUniRegEx(ArabicFatha) + ")(" + MakeUniRegEx(ArabicLetterWaw) + "))" + ")(" + MakeRegMultiEx(Array.ConvertAll(ArabicFathaDammaKasra, Function(Str As String) MakeUniRegEx(Str))) + "))",
                .Evaluator = Nothing}, _
            New RuleMetadataTranslation With {.Rule = "AlefHamza", .Match = "(^\s*|\s+)()(?=" + MakeUniRegEx(ArabicLetterAlefWithHamzaAbove) + "|" + MakeUniRegEx(ArabicLetterAlefWithHamzaBelow) + ")", _
            ...
            New RuleMetadataTranslation With {.Rule = "Maddah", .Match = "(" + MakeUniRegEx(ArabicMaddahAbove) + ")", _
                .Evaluator = New String() {"helpermadda"}}, _
            New RuleMetadataTranslation With {.Rule = "AlefHamzah", .Match = "(" + MakeUniRegEx(ArabicLetterAlefWithHamzaAbove) + "|" + MakeUniRegEx(ArabicLetterAlefWithHamzaBelow) + ")", _
                .Evaluator = New String() {"dividelettersymbol(helperalef, helperhamza)"}}, _
            ...
            New RuleMetadataTranslation With {.Rule = "AlefMaksuraHamzah", .Match = "(" + ArabicFatha + ")(" + MakeUniRegEx(ArabicLetterYehWithHamzaAbove) + ")", _
    ...
    Public Shared Function ReplaceMetadata(ArabicString As String, MetadataRule As RuleMetadata) As String
        For Count As Integer = 0 To ColoringSpelledOutRules.Length - 1
            ...
            If Match <> Nothing Then
                ...
                Dim MetaArgs As String() = System.Text.RegularExpressions.Regex.Match(MetadataRule.Type, Match + "\(.*\)").Value.Split(" ")
                If ColoringSpelledOutRules(Count).RuleFunc <> RuleFuncs.eNone Then
                    ...
                    Else
                        ...
                        For Index As Integer = 0 To Args.Length - 1
                            Str += ReplaceMetadata(Args(Index), New RuleMetadata(0, Args(Index).Length, MetaArgs(Index)))
                        Next
                    End If
                End If
    ...
    Public Shared PlainTransliterateGenJS As String() = {FindLetterBySymbolJS, IsDiacriticJS, _
            ...
            "function replaceMetadata(sVal, metadataRule) { var count, elimParen = function(s) { return s.replace(/\(.*\)/); }; for (count = 0; count < coloringSpelledOutRules.length; count++) { var index, match = null; for (index = 0; index < coloringSpelledOutRules[count].match.split('|'); index++) { if (metadataRule.type.split('|').map(elimParen).indexOf(coloringSpelledOutRules[count].match.split('|')[index]) !== -1) { match = coloringSpelledOutRules[count].match.split('|')[index]; break; } } if (match !== null) { var str = coloringSpelledOutRules[count].evaluator.format(sVal.substr(metadataRule.index, metadataRule.length)); var metaArgs = metadataRule.type.match(/\(.*\)/)[0].split(' '); if (coloringSpelledOutRules[count].ruleFunc !== 0) { var args = ruleFunctions[coloringSpelledOutRules[count].ruleFunc - 1](str); if (args.length === 1) { str = args[0]; } else { str = ''; for (index = 0; index < args.length; index++) { str += replaceMetadata(args[index], {index: 0, length: args[index].length, type: metaArgs[index]}); } } } sVal = sVal.substr(0, metadataRule.index) + str + sVal.substr(metadataRule.index + metadataRule.length); } } return sVal; }", _
