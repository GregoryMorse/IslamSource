...
Public Class Arabic
    ...
    Public Shared RulesOfRecitationRegEx As RuleMetadataTranslation() = { _
        ...
        New RuleMetadataTranslation With {.Rule = "AlefHamza", .Match = "(^\s*)(" + MakeUniRegEx(ArabicLetterAlefWithHamzaAbove) + "|" + MakeUniRegEx(ArabicLetterAlefWithHamzaBelow) + ")", _
    ...
    Public Shared Function TransliterateToPlainRoman(ByVal ArabicString As String) As String
        ...
        For Count = 0 To RulesOfRecitationRegEx.Length - 1
            If Not RulesOfRecitationRegEx(Count).Evaluator Is Nothing Then
                ...
                For MatchIndex As Integer = 0 To Matches.Count - 1
                    For SubCount As Integer = 0 To RulesOfRecitationRegEx(Count).Evaluator.Length - 1
                        If Not RulesOfRecitationRegEx(Count).Evaluator(SubCount) Is Nothing And Matches(MatchIndex).Groups(SubCount + 1).Length <> 0 Then
                            MetadataList.Add(New RuleMetadata(Matches(MatchIndex).Groups(SubCount + 1).Index, Matches(MatchIndex).Groups(SubCount + 1).Length, RulesOfRecitationRegEx(Count).Evaluator(SubCount)))
                        End If
                    Next
                Next
    ...
    Public Shared TransliterateGenJS As String() = {
        ...
        "function doTransliterate(sVal, direction, conversion) { var iCount, iSubCount, sOutVal = ''; for (iCount = 0; iCount < sVal.length; iCount++) { if (sVal.charAt(iCount) === '\\') { iCount++; if (sVal.charAt(iCount) === ',') { sOutVal += String.fromCharCode(1548); } else { sOutVal += sVal.charAt(iCount); } } else { for (iSubCount = 0; iSubCount < arabicLetters.length; iSubCount++) { if (direction ? sVal.charCodeAt(iCount) === parseInt(arabicLetters[iSubCount].Symbol, 10) : sVal.charAt(iCount) === unescape((conversion ? arabicLetters[iSubCount].TranslitLetter : arabicLetters[iSubCount].RomanTranslit))) { sOutVal += (direction ? (conversion ? arabicLetters[iSubCount].TranslitLetter : arabicLetters[iSubCount].RomanTranslit) : (isCombiningSymbol(iSubCount) && (iSubCount === 0 || findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0)) === -1 || !isLetterDiacritic(findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0))) && !isSpecialSymbol(findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0)))) ? String.fromCharCode(0x202D) : '') + String.fromCharCode(arabicLetters[iSubCount].Symbol) + (isCombiningSymbol(iSubCount) && (iSubCount === 0 || findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0)) === -1 || !isLetterDiacritic(findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0))) && !isSpecialSymbol(findLetterBySymbol(sVal.slice(iSubCount - 1, 1).charCodeAt(0)))) ? String.fromCharCode(0x202C) : '')); break; } } if (iSubCount === arabicLetters.length) sOutVal += sVal.charAt(iCount); } } return unescape(sOutVal); }"
    }
