...
Public Class Arabic
    ...
    Public Shared TransliterateGenJS As String() = {
        ...
        "function doTransliterate(sVal, direction, conversion) { var iCount, iSubCount, sOutVal = String.fromCharCode(direction ? 0x200F : 0x200E); if (conversion === 0) return sVal; if (direction && (conversion % 2) === 0) return transliterateWithRules(sVal, Math.floor((conversion - 2) / 2) + 2, generateDefaultStops(sVal), false); for (iCount = 0; iCount < sVal.length; iCount++) { if (sVal.charAt(iCount) === '\\') { iCount++; if (sVal.charAt(iCount) === ',') { sOutVal += String.fromCharCode(0x60C); } else if (sVal.charAt(iCount) === ';') { sOutVal += String.fromCharCode(0x61B); } else if (sVal.charAt(iCount) === '?') { sOutVal += String.fromCharCode(0x61F); } else { sOutVal += String.fromCharCode(0x202D) + sVal.charAt(iCount) + String.fromCharCode(0x202C) + String.fromCharCode(0x202E) + String.fromCharCode(0x202C); } } else { if (getSchemeSpecialFromMatch(sVal.slice(iCount), false) !== -1) { sOutVal += translitSchemes[(Math.floor((conversion - 2) / 2) + 2).toString()].special[getSchemeSpecialFromMatch(sVal.slice(iCount), false)]; re = new RegExp(arabicSpecials[getSchemeSpecialFromMatch(sVal.slice(iCount), false)], ''); iCount += re.exec(sVal.slice(iCount))[0].length - 1; } else if (sVal.length - iCount > 1 && arabicMultis.indexOf(sVal.slice(iCount, 2)) !== -1) { sOutVal += translitSchemes[(Math.floor((conversion - 2) / 2) + 2).toString()].multi[arabicMultis.indexOf(sVal.slice(iCount, 2))]; iCount++; } else { for (iSubCount = 0; iSubCount < arabicLetters.length; iSubCount++) { if (direction ? sVal.charCodeAt(iCount) === parseInt(arabicLetters[iSubCount].Symbol, 10) : sVal.charAt(iCount) === unescape(translitSchemes[(Math.floor((conversion - 2) / 2) + 2).toString()].standard[String.fromCharCode(arabicLetters[iSubCount].Symbol)])) { sOutVal += (direction ? (translitSchemes[(Math.floor((conversion - 2) / 2) + 2).toString()].standard[String.fromCharCode(arabicLetters[iSubCount].Symbol)] ? translitSchemes[(Math.floor((conversion - 2) / 2) + 2).toString()].standard[String.fromCharCode(arabicLetters[iSubCount].Symbol)] : '') : ((isCombiningSymbol(iSubCount) && (iSubCount === 0 || findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1)) === -1 || !isLetterDiacritic(findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1))) && !isSpecialSymbol(findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1))))) ? String.fromCharCode(0x202E) + (true ? '' : String.fromCharCode(0x25CC)) : '') + String.fromCharCode(arabicLetters[iSubCount].Symbol) + ((isCombiningSymbol(iSubCount) && (iSubCount === 0 || findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1)) === -1 || !isLetterDiacritic(findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1))) && !isSpecialSymbol(findLetterBySymbol(sOutVal.charCodeAt(sOutVal.length - 1))))) ? String.fromCharCode(0x202C) : '')); break; } } if (iSubCount === arabicLetters.length) sOutVal += sVal.charAt(iCount); } } } return unescape(sOutVal); }"
    }
...
Public Class TanzilReader
    ...
    Public Shared Function PatternAnalysis() As String()
        ...
        For LetCount = 0 To CachedData.RecitationSymbols.Length - 1
            ...
            For Count As Integer = 0 To Verses.Count - 1
                ...
                Next
            Next
            Strings(LetCount) = ArabicData.LeftToRightOverride + DumpRecDictionary(PreDict, False, 0, True) + "\" + Arabic.TransliterateToScheme(CachedData.RecitationSymbols(LetCount), ArabicData.TranslitScheme.Literal, String.Empty) + "\" + DumpRecDictionary(PostDict, True, 0, True) + ArabicData.PopDirectionalFormatting
        Next
        Return Strings
    End Function
    ...
    Public Shared Function IsQuranTextReference(Str As String) As Boolean
        ...
        Return True
    End Function
    Public Shared Function QuranTextFromReference(Str As String, SchemeType As ArabicData.TranslitScheme, Scheme As String, TranslationIndex As Integer, W4W As Boolean, W4WNum As Boolean, NoArabic As Boolean, Header As Boolean, NoRef As Boolean) As RenderArray
        Dim Renderer As New RenderArray(String.Empty)
        Dim Matches As System.Text.RegularExpressions.MatchCollection = System.Text.RegularExpressions.Regex.Matches(Str, "(?:,?(\d+)(?:\:(\d+))?(?:\:(\d+))?(?:-(\d+)(?:\:(\d+))?(?:\:(\d+))?)?)")
        ...
        For Count = 0 To Matches.Count - 1
            ...
            End If
            If WordNumber = 0 Then WordNumber += 1
            Renderer.Items.AddRange(DoGetRenderedQuranText(QuranTextRangeLookup(BaseChapter, BaseVerse, WordNumber, EndChapter, ExtraVerseNumber, EndWordNumber), BaseChapter, BaseVerse, CachedData.IslamData.Translations.TranslationList(TranslationIndex).Name, SchemeType, Scheme, TranslationIndex, W4W, W4WNum, NoArabic, Header, NoRef).Items)
    ...
    Public Shared Function QuranTextFromSearch(Str As String, SchemeType As ArabicData.TranslitScheme, Scheme As String, TranslationIndex As Integer, W4W As Boolean, W4WNum As Boolean, NoArabic As Boolean, Header As Boolean, NoRef As Boolean) As RenderArray
        Dim Renderer As New RenderArray(String.Empty)
        Dim Verses As List(Of String()) = GetQuranText(CachedData.XMLDocMain, -1, -1, -1, -1)
    ...
    Public Shared Sub CheckMutualExclusiveRules(bAssumeContinue As Boolean)
        Dim Check As String(,) = {{ArabicData.ArabicLetterLam + ArabicData.ArabicShadda + "?", "emphasis|lightness|assimilate|spelllongletter|spelllongmergedletter"}, {"LaamHeaviness", ";;optionalnotstop|optionalstop;optionalstop;;;;emphasis"}, {"LaamLightness", ";lightness;;;;;optionalnotstop;;;lightness;lightness;lightness;lightness;;lightness"}, {"LaamAssimilation", "assimilate"}, {"LaamSeparateLetter", "spelllongletter|spelllongmergedletter"}}
        ...
        Check = {{CachedData.GetPattern("PossibleMadd"), "madd"}, {"MaddahCatcher", ";madd;madd;optionalnotstop;;madd;madd;;optionalnotstop;;madd;optionalnotstop;;madd;;madd;;madd;optionalstop;;madd;;madd;optionalnotstop;;;optionalnotstop;madd;;madd;;madd;madd;;optionalnotstop;madd;optionalnotstop;;madd;optionalnotstop;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd;madd"}, {"MaddahNormal", ";madd;optionalstop;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;madd;;madd;;madd;madd;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;;;madd;;optionalstop|optionalnotstop;optionalnotstop;;;;madd;;madd;;madd;;madd;;optionalnotstop;;;madd;;madd;;madd;;madd;madd;madd;;;madd;optionalstop;;;;madd;;madd;;madd;;madd;;optionalstop;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;madd;;madd;;madd;optionalnotstop;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;optionalstop;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;madd;;;madd;;;madd;madd;;;;madd;;madd;;madd;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;madd;optionalstop"}, {"MaddahExchange", ";madd;;madd;optionalstop;;madd;;;;;;madd;;optionalstop;optionalnotstop;;madd;;madd;madd;optionalstop;;madd;;madd;optionalstop"}, {"MaddahEssentialConnected", ";madd;;madd;;madd;;madd;;;optionalstop|optionalnotstop;optionalnotstop;madd;;madd;;madd;;madd"}, {"MaddahPresentedSukun", ";;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;;madd;;;madd;;;madd;madd;;;;;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;madd;;optionalstop"}, {"MaddahEssentialConnectedPresentedSukun", ";madd;;madd;;madd;;madd;;;optionalstop"}, {"MaddahEssentialSeparate", "madd;optionalnotstop"}, {"MaddahObligatory", "madd"}}
        ...
        'Check = {{ArabicData.ArabicLetterReh + ArabicData.ArabicShadda + "?", "emphasis|lightness|assimilate|spelllongletter"}, {"RaaHeaviness", ";emphasis;;optionalnotstop;;;optionalstop;;emphasis;;emphasis;;emphasis;;emphasis;;optionalstop;emphasis;;optionalnotstop"}, {"RaaLightness", ";lightness;;;;;optionalstop;;lightness;;lightness;;optionalstop;lightness;;optionalnotstop"}, {"RaaHeavinessOrLightness", ";emphasis|lightness;;;optionalnotstop;;;optionalnotstop;;emphasis|lightness;;optionalstop"}, {"RaaAssimilation", "assimilate"}, {"RaaSeparateLetter", ";spelllongletter"}}
        'Check = {{CachedData.GetPattern("Yehs"), "yeh"}, {"NotYeh", "yeh;;yeh;;yeh;;yeh;;yeh;optionalnotstop;yeh;;optionalstop"}, {"MaddYeh", ";;yeh;optionalstop;;optionalstop;yeh;;optionalnotstop;;optionalstop"}, {"LeenYeh", ";yeh;;yeh;;optionalstop"}, {"StrongYeh", ";yeh;;;yeh;;;yeh;;yeh;yeh;;;yeh;;yeh;;yeh;;yeh;;optionalnotstop;;yeh;;yeh;;yeh;;optionalnotstop"}}
        'Check = {{CachedData.GetPattern("Waws"), "waw"}, {"NotWaw", "waw;;waw;;waw;;optionalnotstop;;optionalstop;;waw;;waw;optionalnotstop;;waw;;optionalstop"}, {"MaddWaw", ";waw;optionalstop;;optionalstop;;waw;;optionalnotstop"}, {"LeenWaw", ";waw;;optionalstop;;optionalstop;;optionalnotstop;waw"}, {"StrongWaw", ";;optionalnotstop;;waw;;;waw;;waw;;optionalstop;waw;;waw;;optionalnotstop;;waw;;optionalnotstop"}}
        Dim IndexToVerse As Integer()() = Nothing
            ...
            For Count = 0 To Matches.Count - 1
                ...
                For SubCount = 0 To MetaRules.Length - 1
                    If Array.IndexOf(MetaRules(SubCount).Split("|"c), If(bAssumeContinue, "optionalstop", "optionalnotstop")) <> -1 And Matches(Count).Groups(SubCount + 1).Success Then
                        ...
                        Next
                        If Matches(Count).Index + CheckLen <> Matches(Count).Groups(SubCount + 1).Index Then Debug.Print("Non-Sequential Capture:" + CStr(MainCount) + ":" + Arabic.TransliterateToScheme(Text.Substring(Math.Max(0, Matches(Count).Groups(SubCount + 1).Index - 15), 30), ArabicData.TranslitScheme.Literal, String.Empty))
                        bSieve = True
                        Exit For
                    End If
                Next
    ...
    Public Shared Function GetRenderedQuranText(ByVal Item As PageLoader.TextItem) As RenderArray
        ...
        Return GetQuranTextBySelection(Item.Name, Division, Index, CachedData.IslamData.Translations.TranslationList(TranslationIndex).FileName, SchemeType, Scheme, TranslationIndex, True, False, False, True, False)
    End Function
            ...
            For Chapter = 0 To QuranText.Count - 1
                ...
                For Verse = 0 To QuranText(Chapter).Length - 1
                    ...
                    If W4W And Translation <> String.Empty Then
                        ...
                        Dim TranslitWords As String() = Arabic.TransliterateToScheme(QuranText(Chapter)(Verse), SchemeType, Scheme, GenerateDefaultStops(QuranText(Chapter)(Verse))).Split(" "c)
