...
Public Class Arabic
    ...
    Public Shared Function GetTransliterationSchemes() As Array()
        ...
        For Count = 0 To CachedData.IslamData.TranslitSchemes.Length - 1
            Strings(Count + 2) = New String() {Utility.LoadResourceString("IslamSource_" + CachedData.IslamData.TranslitSchemes(Count).Name), CStr(Count + 2)}
        Next
        Return Strings
    End Function
    Public Shared Function GetChangeTransliterationJS() As String()
        ...
        "function changeTransliteration() { var k, child, iSubCount, text; $('span.transliteration').each(function() { $(this).css('display', $('#translitscheme').val() === '0' ? 'none' : 'block'); }); for (k in renderList) { text = ''; for (child in renderList[k]['children']) { for (iSubCount = 0; iSubCount < renderList[k]['children'][child]['arabic'].length; iSubCount++) { if ($('#translitscheme').val() !== '0' && $('#translitscheme').val() !== '1' && renderList[k]['children'][child]['arabic'][iSubCount] !== '' && (renderList[k]['children'][child]['arabic'][iSubCount].length !== 1 || !isStop(findLetterBySymbol(renderList[k]['children'][child]['arabic'][iSubCount].charCodeAt(0)))) && renderList[k]['children'][child]['translit'][iSubCount] !== '') { if (text !== '') text += ' '; text += $('#' + renderList[k]['children'][child]['arabic'][iSubCount]).text(); } else { if (renderList[k]['children'][child]['translit'][iSubCount] !== '') $('#' + renderList[k]['children'][child]['translit'][iSubCount]).text(($('#translitscheme').val() === '0' || renderList[k]['children'][child]['arabic'][iSubCount] === '') ? '' : doTransliterate($('#' + renderList[k]['children'][child]['arabic'][iSubCount]).text(), true, parseInt($('#translitscheme').val(), 10))); } } } if ($('#translitscheme').val() !== '0' && $('#translitscheme').val() !== '1') { text = transliterateWithRules(text, parseInt($('#translitscheme').val(), 10)).split(' '); for (child in renderList[k]['children']) { for (iSubCount = 0; iSubCount < renderList[k]['children'][child]['translit'].length; iSubCount++) { if (renderList[k]['children'][child]['translit'][iSubCount] !== '') $('#' + renderList[k]['children'][child]['translit'][iSubCount]).text(text.shift()); } } } for (iSubCount = 0; iSubCount < renderList[k]['arabic'].length; iSubCount++) { if (renderList[k]['translit'][iSubCount] !== '') $('#' + renderList[k]['translit'][iSubCount]).text(($('#translitscheme').val() === '0' || renderList[k]['arabic'][iSubCount] === '') ? '' : (($('#translitscheme').val() !== '0' && $('#translitscheme').val() !== '1') ? transliterateWithRules($('#' + renderList[k]['arabic'][iSubCount]).text(), parseInt($('#translitscheme').val(), 10)) : doTransliterate($('#' + renderList[k]['arabic'][iSubCount]).text(), true, parseInt($('#translitscheme').val(), 10)))); } } }"}
        GetJS.AddRange(TransliterateGenJS)
        GetJS.AddRange(NumberGenJS)
...
Public Class Quiz
    ...
    Public Shared Function VerifyAnswer() As String()
        Dim JSList As New List(Of String) From {"javascript: verifyAnswer(this);", String.Empty, _
            ...
            "function getQA(quizSet, quest, nidx) { if (quest) return quizSet[nidx]; return $('#translitscheme').val() === '0' ? transliterateWithRules(quizSet[nidx], parseInt($('#translitscheme').val(), 10)) : doTransliterate(quizSet[nidx], true, parseInt($('#translitscheme').val(), 10) + 1); }", _
            ...
            "function verifyAnswer(ctl) { $(ctl).prop('value') === ($('#translitscheme').val() === '0' ? transliterateWithRules($('#quizquestion').text().trim(), parseInt($('#translitscheme').val(), 10)) : doTransliterate($('#quizquestion').text().trim(), true, parseInt($('#translitscheme').val(), 10) + 1)) ? qright++ : qwrong++; nextQuestion(); }"}
