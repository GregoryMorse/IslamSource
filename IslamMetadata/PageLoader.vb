...
Public Class CachedData
    ...
    Shared _ErrorCheckRules As Nullable(Of IslamData.RuleTranslationCategory)
    Public Shared Function GetRuleSet(Name As String) As Integer
        Dim Count As Integer
        For Count = 0 To CachedData.IslamData.RuleSets.Length - 1
            If CachedData.IslamData.RuleSets(Count).Name = Name Then Return Count
        Next
        Return -1
    End Function
    ...
    Public Shared ReadOnly Property ErrorCheckRules As IslamData.RuleTranslationCategory.RuleTranslation()
        Get
            If Not _ErrorCheckRules.HasValue Then
                Dim RuleIndex As Integer = GetRuleSet("ErrorCheck")
                _ErrorCheckRules = CachedData.IslamData.RuleSets(RuleIndex)
                For Count As Integer = 0 To _ErrorCheckRules.Value.Rules.Length - 1
                    _ErrorCheckRules.Value.Rules(Count).Match = TranslateRegEx(_ErrorCheckRules.Value.Rules(Count).Match)
                Next
            End If
            Return _ErrorCheckRules.Value.Rules
        End Get
    End Property
