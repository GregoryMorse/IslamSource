...
Public Class Arabic
    ...
    End Function
    Public Function TransliterateToScheme(ByVal ArabicString As String, SchemeType As ArabicData.TranslitScheme, Scheme As String, MetadataList As Generic.List(Of FullRuleMetadata)) As String
    ...
    End Function
    Class FullRuleMetadata
        Sub New()
        End Sub
        Sub New(NewIndex As Integer, NewLength As SByte, ByVal NewType As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs(), NewOrigOrder As Short)
            Index = NewIndex
            Length = NewLength
            Type = NewType
        ...
        Public Length As SByte
        Public Type As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs()
        Public OrigOrder As Short
        Public Children As FullRuleMetadata()
        Public Dependencies As FullRuleMetadata()
    End Class
    Class RuleMetadata
        Sub New()
        End Sub
        Sub New(NewIndex As Integer, NewLength As SByte, ByVal NewType As IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex, NewOrigOrder As Short)
            Index = NewIndex
            Length = NewLength
            Type = NewType
            OrigOrder = NewOrigOrder
        ...
        Public Index As Integer
        Public Length As SByte
        Public Type As IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex
        Public OrigOrder As Short
    ...
    Class RuleMetadataComparer
        Implements Collections.Generic.IComparer(Of FullRuleMetadata)
        Public Function Compare(x As FullRuleMetadata, y As FullRuleMetadata) As Integer Implements Generic.IComparer(Of FullRuleMetadata).Compare
    ...
    Public Shared Function ProcessTransform(ArabicString As String, Rules As IslamData.RuleTranslationCategory.RuleTranslation(), bPriority As Boolean) As String
        ...
        Dim Replacements As New List(Of FullRuleMetadata)
                ...
                For MatchCount = 0 To Matches.Count - 1
                    If Rules(Count).NegativeMatch = String.Empty OrElse Matches(MatchCount).Result(Rules(Count).NegativeMatch) = String.Empty Then
                        ...
                        For DupCount = 0 To Matches(MatchCount).Result(Rules(Count).Evaluator).Length - 1
                            ...
                            End If
                        Next
                        Replacements.Add(New FullRuleMetadata(Matches(MatchCount).Index + DupCount, CSByte(Matches(MatchCount).Length - DupCount), New IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs() {New IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs With {.RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(Matches(MatchCount).Result(Rules(Count).Evaluator).Substring(DupCount))}}, CShort(Count)))
                    End If
                Next
            End If
        Next
        ...
        For Count = 0 To Replacements.Count - 1
            ...
            End If
            ArabicString = ArabicString.Substring(0, Replacements(Count).Index) + IslamData.RuleMetaSet.RuleMetadataTranslation.GetString(Replacements(Count).Type(0).RuleName) + ArabicString.Substring(Replacements(Count).Index + Replacements(Count).Length)
        Next
        Return ArabicString
    End Function
    ...
    Public Function ChangeScript(ArabicString As String, SrcScriptType As TanzilReader.QuranScripts, ScriptType As TanzilReader.QuranScripts, ByVal PreString As String, ByVal PostString As String) As String
        ...
        Return ArabicString
    End Function
    Public Function ReplaceMetadata(ArabicString As String, MetadataRule As FullRuleMetadata, LearningMode As Boolean) As String
        If MetadataRule.Type.Length = 0 OrElse MetadataRule.Type.Length = 1 And MetadataRule.Type(0).RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(String.Empty) Then Return ArabicString
    ...
    Class FullFilterMetadataComparer
        Implements IComparer(Of FullRuleMetadata)
        Public Function Compare(x As FullRuleMetadata, y As FullRuleMetadata) As Integer Implements IComparer(Of FullRuleMetadata).Compare
            If x.Index = y.Index Then Return 0
            Return If(x.Index > y.Index, -1, 1)
        End Function
    End Class
    Public Shared Function FilterMetadataStopsContig(ByVal ArabicStringLength As Integer, MetadataList As Generic.List(Of FullRuleMetadata), OptionalStops() As Integer, PreStringLength As Integer, Optional PreStop As Boolean = True, Optional PostStop As Boolean = True) As Generic.List(Of FullRuleMetadata)
        Dim Idx As Integer = MetadataList.BinarySearch(New FullRuleMetadata With {.Index = PreStringLength}, New FullFilterMetadataComparer)
    ...
    Public Shared Function FilterMetadataStops(ByVal ArabicString As String, MetadataList As Generic.List(Of FullRuleMetadata), OptionalStops() As Integer) As Generic.List(Of FullRuleMetadata)
        ...
        While Index <= TransformMetadataStops.Count - 1
            Do While Index <> TransformMetadataStops.Count - 1 AndAlso TransformMetadataStops(Index).Index = TransformMetadataStops(Index + 1).Index AndAlso TransformMetadataStops(Index).Length = TransformMetadataStops(Index + 1).Length AndAlso CompareRuleMetadata(TransformMetadataStops(Index).Children, TransformMetadataStops(Index + 1).Children)
                Dim FirstRule As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs() = TransformMetadataStops(Index).Type
                Dim SecondRule As New List(Of IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs)
                SecondRule.AddRange(TransformMetadataStops(Index + 1).Type)
                ...
                'Debug.WriteLine("First: " + FilterMetadataStops(Index).Type + " Second: " + FilterMetadataStops(Index + 1).Type + " After: " + String.Join("|"c, SecondRule.ToArray()))
                TransformMetadataStops(Index + 1) = New FullRuleMetadata(TransformMetadataStops(Index).Index, TransformMetadataStops(Index).Length, SecondRule.ToArray(), TransformMetadataStops(Index).OrigOrder) With {.Children = TransformMetadataStops(Index).Children, .Dependencies = TransformMetadataStops(Index).Dependencies}
    ...
    Public Class RuleIndexComparer
        Implements IComparer(Of RuleMetadata)
        Dim _PosDict As Dictionary(Of RuleMetadata, Integer)
        Public Sub New(PosDict As Dictionary(Of RuleMetadata, Integer))
            _PosDict = PosDict
        End Sub
        Public Function Compare(x As RuleMetadata, y As RuleMetadata) As Integer Implements IComparer(Of RuleMetadata).Compare
            If x.Index = y.Index Then Return If(_PosDict(x) = _PosDict(y), If(x.OrigOrder = y.OrigOrder, 0, If(x.OrigOrder > y.OrigOrder, 1, -1)), If(_PosDict(x) > _PosDict(y), 1, -1))
            Return If(x.Index > y.Index, -1, 1)
        End Function
    End Class
    Class RuleMetadataEqualityComparer
        ...
        Public Overloads Function GetHashCode(obj As RuleMetadata) As Integer Implements IEqualityComparer(Of RuleMetadata).GetHashCode
            ...
            For Count = 0 To obj.Type.Length - 1
                GetHashCode = GetHashCode Xor IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs(obj.Type.Index + Count).RuleName.GetHashCode()
                If Not IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs(obj.Type.Index + Count).Args Is Nothing Then
                    For SubCount = 0 To IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs(obj.Type.Index + Count).Args.Length - 1
                        For SubArgCount = 0 To IslamData.RuleMetaSet.RuleMetadataTranslation.Args(IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs(obj.Type.Index + Count).Args.Index + SubCount).Length - 1
                            GetHashCode = GetHashCode Xor IslamData.RuleMetaSet.RuleMetadataTranslation.Strs(IslamData.RuleMetaSet.RuleMetadataTranslation.Args(IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs(obj.Type.Index + Count).Args.Index + SubCount).Index + SubArgCount).GetHashCode()
                        Next
                    Next
                End If
            Next
    ...
    Public Shared Function SelectSplit(Of T)(str As String, separator As Char, ByRef sepList As List(Of Integer), selector As Func(Of String, T)) As T()
        sepList.Clear()
        ...
        Do While ((j < str.Length) AndAlso (sepList.Count < length))
            ...
            j += 1
        Loop
        If (sepList.Count = 0) Then Return New T() {selector(str)}
        Dim startIndex As Integer = &H0
        Dim index As Integer = &H0
        Dim arr As T() = New T((sepList.Count + &H1) - 1) {}
        Dim i As Integer = &H0
    ...
    Public Shared Function QuadSplit(str As String, separator As Char, ByRef str0 As String, ByRef str1 As String, ByRef str2 As String, ByRef str3 As String) As Boolean
        ...
        Return True
    End Function
    Public Shared Function IntParseFast(value As String) As Integer
        Dim result As Integer = 0
        Dim length As Integer = value.Length
        For i As Integer = 0 To length - 1
            result = 10 * result + Convert.ToInt32(value.Chars(i)) - 48
        Next
        Return result
    End Function
    Public Shared Function GetCacheMetarules(Lines As String(), IndexToVerse As Integer()()) As Generic.List(Of RuleMetadata)
        ...
        Dim TempInnerCount As Integer
        Dim TempOuterCount As Integer
        ...
        Dim sepListInInInIn As List(Of Integer) = New List(Of Integer)
        For Count As Integer = 0 To Lines.Length - 1
            ...
            Dim Key As IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() = SelectSplit(KeyVal.Substring(0, Math.Min(If(FirstBrace = -1, KeyVal.Length, FirstBrace), If(FirstCurlyBrace = -1, KeyVal.Length, FirstCurlyBrace))), "|"c, sepList, Function(S)
                                                                                                                                                                                                                                                              ...
                                                                                                                                                                                                                                                              If M.Success AndAlso Not String.IsNullOrEmpty(M.Groups(1).Value) Then
                                                                                                                                                                                                                                                                  IslamData.RuleMetaSet.RuleMetadataTranslation.Args.AddRange(SelectSplit(M.Groups(1).Value, ","c, sepListIn, Function(InnerStr)
                                                                                                                                                                                                                                                                                                                                                                                  TempCount = IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.Count
                                                                                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                                                                                  Return New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = TempCount, .Length = CShort(TempCount - IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.Count)}
                                                                                                                                                                                                                                                                                                                                                                              End Function))
                                                                                                                                                                                                                                                              End If
                ...
                Dim Ch As RuleMetadata() = If(FirstBrace = -1, Nothing, SelectSplit(Childs, "&"c, sepList, Function(Item)
                                                                                                               ...
                                                                                                               If Not String.IsNullOrEmpty(NewType) Then
                                                                                                                   IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.AddRange(SelectSplit(NewType, "+"c, sepListIn, Function(S)
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        End Function))
                                                                                                               End If
                                                                                                               Return New RuleMetadata(IntParseFast(NewIndex), CSByte(NewLength), If(String.IsNullOrEmpty(NewType), Nothing, New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = TempCount, .Length = CShort(IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.Count - TempCount)}), CShort(IntParseFast(NewOrigOrder))) With {.Dependencies = CDep}
                                                                                                           End Function))
    ...
    Public Shared Function MakeCacheMetarules(RuleMetadata As Generic.List(Of RuleMetadata), IndexToVerse As Integer()()) As String()
        ...
        Dim RepCt As Integer
        For Count As Integer = 0 To RuleMetadata.Count - 1
            Dim Idx As Integer = Count
            If Count <> 0 AndAlso RuleMetadata(Count).Index = RuleMetadata(Count - 1).Index Then
                RepCt += 1
            Else
                RepCt = 0
            End If
    ...
    Public Class MetaruleComparer
        ...
        Public Function Compare(x As RuleMetadata, y As RuleMetadata) As Integer Implements IComparer(Of RuleMetadata).Compare
            If x.Index = y.Index Then Return 0
            Return If(x.Length = -1 AndAlso x.Index >= y.Index OrElse Not (y.Length = -1 AndAlso y.Index >= x.Index), -1, 1)
        End Function
    End Class
    ...
    Public Shared Function GetMetarulesFromCache(ArabicString As String, BasePosition As Integer, Rules As Generic.List(Of RuleMetadata)) As Generic.List(Of FullRuleMetadata)
        ...
        If Index < 0 Then Index = (Index Xor -1) - 1
    ...
    Public Shared Function CompareRuleMetadata(RuleMetadata1() As FullRuleMetadata, RuleMetadata2() As FullRuleMetadata) As Boolean
        ...
        Return True
    End Function
    Public Function GetMetarules(ArabicString As String, RuleSet As IslamMetadata.IslamData.RuleMetaSet) As Generic.List(Of FullRuleMetadata)
        Dim Count As Integer
        Dim MetadataList As New Generic.List(Of FullRuleMetadata)
        For Count = 0 To RuleSet.Rules.Length - 1
            If Not RuleSet.Rules(Count).Evaluator Is Nothing Then
                ...
                For MatchIndex = 0 To Matches.Count - 1
                    ...
                    Dim ParentDependency As New List(Of FullRuleMetadata)
        ...
        While Index <= MetadataList.Count - 1
            Do While Index <> MetadataList.Count - 1 AndAlso MetadataList(Index).Index = MetadataList(Index + 1).Index AndAlso MetadataList(Index).Length = MetadataList(Index + 1).Length AndAlso CompareRuleMetadata(MetadataList(Index).Dependencies, MetadataList(Index + 1).Dependencies) AndAlso CompareRuleMetadata(MetadataList(Index).Children, MetadataList(Index + 1).Children)
                ...
                MetadataList(Index + 1) = New FullRuleMetadata(MetadataList(Index).Index, MetadataList(Index).Length, SecondRule.ToArray(), MetadataList(Index).OrigOrder) With {.Children = MetadataList(Index).Children, .Dependencies = MetadataList(Index).Dependencies}
                RemoveIndexes(Index) = -1
                Index += 1
            Loop
            Index += 1
...
Public Class IslamData
    ...
    Class RuleMetaSet
        Class RuleMetadataTranslation
            ...
            Class RuleWithArgs
                Public RuleName As Short
                Public Args As BeginEndIndex
            End Class
            Class FullRuleWithArgs
                Public RuleName As Short
                Public Args As Short()()
            End Class
            Public Shared Args As New List(Of BeginEndIndex)
            Public Shared Strs As New List(Of Short)
            Class BeginEndIndex
                Public Index As Integer
            ...
            ReadOnly Property Evaluator As BeginEndIndex
                ...
                End Get
            End Property
            Public Shared RulesWithArgs As New List(Of BeginEndIndex)
            Public Shared AllArgs As New List(Of RuleWithArgs)
            Public Shared _StringTable As New Dictionary(Of String, Short)
            Public Shared Function GetStringIndex(Str As String) As Short
                If Not _StringTable.ContainsKey(Str) Then _StringTable.Add(Str, CShort(_StringTable.Count))
                Return _StringTable(Str)
            End Function
            Public Shared Function GetString(Index As Short) As String
                For Each KV As KeyValuePair(Of String, Short) In _StringTable
                    If KV.Value = Index Then Return KV.Key
                Next
                Return Nothing
            End Function
    ...
    Class QuranSelection
        Class QuranSelectionInfo
            ...
            <ComponentModel.DefaultValueAttribute(-2)>
...
Public Class CachedData
    ...
    Public Class HadithDocInfo
        Class Books
            ...
            <Xml.Serialization.XmlAttribute("sharedhadiths")>
            Public SharedHadiths As Integer
            <Xml.Serialization.XmlAttribute("sourced")>
...
Public Class TanzilReader
    ...
    Public Async Function MakeQuranCacheMetarules() As Threading.Tasks.Task(Of List(Of Arabic.RuleMetadata))
        Dim Rules As List(Of Arabic.RuleMetadata) = Linq.Enumerable.Select(Arb.GetMetarules(_QuranText, ChData.RuleMetas("UthmaniQuran")), Function(It As Arabic.FullRuleMetadata)
                                                                                                                                               Dim RulesWithArgs As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs) = Linq.Enumerable.Select(It.Type, Function(Rule As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs)
                                                                                                                                                                                                                                                                                     Dim IndRules As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex) = Linq.Enumerable.Select(Rule.Args, Function(Arg As Short())
                                                                                                                                                                                                                                                                                                                                                                                                                         IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.AddRange(Arg)
                                                                                                                                                                                                                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                                                                                                                                                                                                                     End Function)
                                                                                                                                                                                                                                                                                     IslamData.RuleMetaSet.RuleMetadataTranslation.Args.AddRange(IndRules)
                                                                                                                                                                                                                                                                                     Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = Rule.RuleName, .Args = New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Args.Count - IndRules.Count, .Length = CShort(IndRules.Count)}}
                                                                                                                                                                                                                                                                                 End Function)
                                                                                                                                               IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.AddRange(RulesWithArgs)
                                                                                                                                               Return New Arabic.RuleMetadata(It.Index, It.Length, New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.Count - RulesWithArgs.Count, .Length = CShort(RulesWithArgs.Count)}, It.OrigOrder) With {.Children = Linq.Enumerable.Select(It.Children, Function(ChildIt As Arabic.FullRuleMetadata)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       Dim OthRulesWithArgs As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs) = Linq.Enumerable.Select(ChildIt.Type, Function(Rule As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     Dim IndRules As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex) = Linq.Enumerable.Select(Rule.Args, Function(Arg As Short())
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.AddRange(Arg)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         Return New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.Count - Arg.Length, .Length = CShort(Arg.Length)}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = Rule.RuleName, .Args = New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Args.Count - IndRules.Count, .Length = CShort(IndRules.Count)}}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.AddRange(OthRulesWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       Return New Arabic.RuleMetadata(ChildIt.Index, ChildIt.Length, New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.Count - OthRulesWithArgs.Count, .Length = CShort(OthRulesWithArgs.Count)}, ChildIt.OrigOrder) With {.Dependencies = Linq.Enumerable.Select(ChildIt.Children, Function(DepIt As Arabic.FullRuleMetadata)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             Dim OthDepRulesWithArgs As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs) = Linq.Enumerable.Select(DepIt.Type, Function(Rule As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            Dim IndRules As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex) = Linq.Enumerable.Select(Rule.Args, Function(Arg As Short())
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.AddRange(Arg)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                Return New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.Count - Arg.Length, .Length = CShort(Arg.Length)}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = Rule.RuleName, .Args = New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Args.Count - IndRules.Count, .Length = CShort(IndRules.Count)}}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.AddRange(OthDepRulesWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             Return New Arabic.RuleMetadata(DepIt.Index, DepIt.Length, New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.Count - OthDepRulesWithArgs.Count, .Length = CShort(OthDepRulesWithArgs.Count)}, DepIt.OrigOrder)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         End Function).ToArray()}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   End Function).ToArray(), .Dependencies = Linq.Enumerable.Select(It.Dependencies, Function(DepIt As Arabic.FullRuleMetadata)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        Dim OthRulesWithArgs As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs) = Linq.Enumerable.Select(DepIt.Type, Function(Rule As IslamData.RuleMetaSet.RuleMetadataTranslation.FullRuleWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    Dim IndRules As IEnumerable(Of IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex) = Linq.Enumerable.Select(Rule.Args, Function(Arg As Short())
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.AddRange(Arg)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        Return New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.Strs.Count - Arg.Length, .Length = CShort(Arg.Length)}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                End Function)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.AddRange(OthRulesWithArgs)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        Return New Arabic.RuleMetadata(DepIt.Index, DepIt.Length, New IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex() With {.Index = IslamData.RuleMetaSet.RuleMetadataTranslation.AllArgs.Count - OthRulesWithArgs.Count, .Length = CShort(OthRulesWithArgs.Count)}, DepIt.OrigOrder)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    End Function).ToArray()}
                                                                                                                                           End Function).ToList()
    ...
    Public Async Function GetQuranCacheMetarules() As Threading.Tasks.Task(Of List(Of Arabic.RuleMetadata))
        Dim Path As String = _PortableMethods.Settings.GetFilePath(_PortableMethods.FileIO.CombinePath("metadata", "QuranTajweedData.txt"))
    ...
    Public Async Function PatternMatch(BaseText As QuranTexts, ScriptType As QuranScripts, Presentation As ArabicPresentation, Pattern As String) As Threading.Tasks.Task(Of List(Of String))
        ...
        Dim Doc As CachedData.QuranDocMain
        ...
        End If
        Dim xs As Xml.Serialization.XmlSerializer = New Xml.Serialization.XmlSerializer(GetType(CachedData.QuranDocMain))
        Doc = CType(xs.Deserialize(Stream), CachedData.QuranDocMain)
        Stream.Dispose()
        Dim Verses As Collections.Generic.List(Of String())
        Verses = GetQuranText(Doc, -1, -1, -1, -1)
        For Count As Integer = 0 To Verses.Count - 1
            Dim ChapterNode As CachedData.QuranDocMain.Sura = GetTextChapter(Doc, Count + 1)
            For SubCount As Integer = 0 To Verses(Count).Length - 1
                If SubCount = 0 AndAlso Not String.IsNullOrEmpty(GetTextVerse(ChapterNode, SubCount + 1).Bismillah) Then
                    For Each Val As System.Text.RegularExpressions.Match In System.Text.RegularExpressions.Regex.Matches(GetTextVerse(ChapterNode, SubCount + 1).Bismillah, Pattern)
    ...
    Public Async Function CompareQuranFormats(BaseText As QuranTexts, TargetBaseText As QuranTexts, ScriptType As QuranScripts, Presentation As ArabicPresentation) As Threading.Tasks.Task
        ...
        Doc = CType(xs.Deserialize(Stream), CachedData.QuranDocMain)
        Stream.Dispose()
        Dim TargetDoc As CachedData.QuranDocMain
        ...
        Else
            Stream = Await _PortableMethods.FileIO.LoadStream(_PortableMethods.Settings.GetFilePath(_PortableMethods.FileIO.CombinePath("metadata", QuranTextNames(TargetBaseText) + "-" + QuranFileNames(ScriptType) + If(Presentation <> ArabicPresentation.None, "-" + PresentationCacheNames(Presentation), String.Empty) + ".xml")))
        End If
        TargetDoc = CType(xs.Deserialize(Stream), CachedData.QuranDocMain)
        Stream.Dispose()
        Dim Verses As Collections.Generic.List(Of String())
        ...
        Next
    End Function
    Public Shared Function GetTextChapterXml(ByVal XMLDocMain As Xml.Linq.XDocument, ByVal Chapter As Integer) As Xml.Linq.XElement
        Return Utility.GetChildNodeByIndex("sura", "index", Chapter, New List(Of Xml.Linq.XElement)(XMLDocMain.Root.Elements).ToArray())
    End Function
    Public Shared Function GetTextVerseXml(ByVal ChapterNode As Xml.Linq.XElement, ByVal Verse As Integer) As Xml.Linq.XElement
    ...
    Public Async Function ChangeQuranFormat(BaseText As QuranTexts, TargetBaseText As QuranTexts, SrcScriptType As QuranScripts, ScriptType As QuranScripts, Presentation As ArabicPresentation) As Threading.Tasks.Task
        ...
        Dim xs As Xml.Serialization.XmlSerializer = New Xml.Serialization.XmlSerializer(GetType(CachedData.QuranDocMain))
        Dim DocMain As CachedData.QuranDocMain = CType(xs.Deserialize(Stream), CachedData.QuranDocMain)
        Stream.Dispose()
        Dim Verses As Collections.Generic.List(Of String())
        ...
        Verses = GetQuranText(DocMain, -1, -1, -1, -1)
        For Count As Integer = 0 To Verses.Count - 1
            Dim VerseAdjust As Integer = 0
            Dim ChapterNode As CachedData.QuranDocMain.Sura = GetTextChapter(DocMain, Count + 1)
            Dim ChapterNodeXml As Xml.Linq.XElement = GetTextChapterXml(Doc, Count + 1)
            If UseBuckwalter Then
                ChapterNodeXml.Attribute("name").Value = Arb.TransliterateToScheme(ChapterNode.Name, ArabicData.TranslitScheme.Literal, String.Empty, Nothing)
            End If
            Dim SubCount As Integer = 0
            While SubCount <= Verses(Count).Length - 1 - VerseAdjust
                Dim CurVerse As Xml.Linq.XElement = GetTextVerseXml(ChapterNodeXml, SubCount + 1)
                ...
                If SubCount = 0 AndAlso Not CurVerse.Attribute("bismillah") Is Nothing Then
                    If Count <> 0 Then
                        PreVerse = GetTextVerse(GetTextChapter(DocMain, Count), GetTextChapter(DocMain, Count).Ayas.Length).Text
                ...
                If SubCount + 1 <= Verses(Count).Length - 1 - VerseAdjust Then
                    NextVerse = GetTextVerse(ChapterNode, SubCount + 1 + 1).Text
                ElseIf Count <> Verses.Count - 1 Then
                    NextVerse = If(String.IsNullOrEmpty(GetTextVerse(GetTextChapter(DocMain, Count + 1 + 1), 1).Bismillah), GetTextVerse(GetTextChapter(DocMain, Count + 1 + 1), 1).Text, GetTextVerse(GetTextChapter(DocMain, Count + 1 + 1), 1).Bismillah)
                End If
                If Count <> 0 AndAlso SubCount = 0 AndAlso CurVerse.Attribute("bismillah") Is Nothing Then
                    PreVerse = GetTextVerse(GetTextChapter(DocMain, Count), GetTextChapter(DocMain, Count).Ayas.Length).Text
                ElseIf SubCount <> 0 Then
                    PreVerse = GetTextVerse(ChapterNode, SubCount).Text
                End If
                If UseBuckwalter Then PreVerse = Arb.TransliterateFromBuckwalter(PreVerse)
                ...
                If BaseText = QuranTexts.Hafs And TargetBaseText = QuranTexts.Warsh Then
                    ...
                    If Index <> ChData.IslamData.VerseNumberSchemes(0).CombinedVerses.Length Then
                        If Count = 0 And SubCount = 1 Then
                            ...
                            GetTextVerseXml(ChapterNodeXml, SubCount).Add(NewAttr)
                            GetTextVerseXml(ChapterNodeXml, SubCount).Attribute("text").Value = CurVerse.Attribute("text").Value
                        Else
                            GetTextVerseXml(ChapterNodeXml, SubCount).Attribute("text").Value = GetTextVerse(ChapterNode, SubCount).Text + " " + CurVerse.Attribute("text").Value
                        End If
                        CurVerse.Remove()
                        CurVerse = GetTextVerseXml(ChapterNodeXml, SubCount)
                        VerseAdjust += 1
                        SubCount -= 1
    ...
    Public Function IsQuranTextReference(Str As String) As Boolean
        ...
        For Count = 0 To Matches.Count - 1
            ...
            If BaseVerse = 0 Then
                BaseVerse += 1
                ExtraVerseNumber = GetTextChapter(ChData.XMLDocMain, BaseChapter).Ayas.Length
            End If
    ...
    Public Async Function CheckSequentialRules() As Threading.Tasks.Task
        ...
        Dim IndexToVerse As Integer()() = Nothing
        Dim XMLDocAlt As CachedData.QuranDocMain
        ...
        Dim xs As Xml.Serialization.XmlSerializer = New Xml.Serialization.XmlSerializer(GetType(CachedData.QuranDocMain))
        XMLDocAlt = CType(xs.Deserialize(Stream), CachedData.QuranDocMain)
        Stream.Dispose()
    ...
    Public Sub CheckMutualExclusiveRules(bAssumeContinue As Boolean, VerIndex As Integer)
        ...
        For MainCount = 0 To ChData.IslamData.VerificationSet(VerIndex).MetaRules.Length - 1
            ...
            Dim MetaRules As IslamData.RuleMetaSet.RuleMetadataTranslation.BeginEndIndex = If(bVerSet, ChData.RuleMetas("Verification").Rules(MetaCount).Evaluator, ChData.RuleMetas("UthmaniQuran").Rules(MetaCount).Evaluator)
            ...
            If (Verses.Count = 0 And BaseVerse = 0 And ExtraVerseNumber = 0 Or
                ...
                If Not String.IsNullOrEmpty(Node) Then
                    ...
                    For WordCount = 0 To Matches.Count - 1
                        ...
                        Else
                            If bNotFilter Then IndexToVerseList.Add(New Integer() {StartChapter - If(bChapterRollback, 1, 0) + Count, 0, WordCount - MarkerCount + 1, Str.Length + Matches(WordCount).Groups(1).Index - FilterIndex, Matches(WordCount).Groups(1).Length, ChunkCount, 0})
                        End If
                ...
                For SubCount As Integer = 0 To Verses(Count).Length - 1
                    ...
                    For WordCount = 0 To Matches.Count - 1
                        ...
                        If Matches(WordCount).Groups(2).Success Then
                            ...
                            If WordCount <> 0 AndAlso Not Matches(WordCount - 1).Groups(2).Success Then ChunkCount += 1
                        Else
                            If bNotFilter Then IndexToVerseList.Add(New Integer() {StartChapter - If(bChapterRollback, 1, 0) + Count, If(Count <> 0, 1, If(BaseVerse = 0, 1, BaseVerse) - If(_CacheMetarules Is Nothing And WordNumber = 1 And Not bBismillahPrecedes And Not (StartChapter = 1 And BaseVerse <= 1), 1, 0)) + SubCount, WordCount - MarkerCount + 1, Str.Length + Matches(WordCount).Groups(1).Index - FilterIndex, Matches(WordCount).Groups(1).Length, ChunkCount, 0})
                        End If
                        If Not bNotFilter Then FilterIndex = Matches(WordCount).Index + Matches(WordCount).Length + 1
                    Next
                    If WordCount <> Matches.Count Then
    ...
    Public Class QuranWordChapterVerseWordComparer
        ...
        Public Function Compare(x As Integer(), y As Integer()) As Integer Implements IComparer(Of Integer()).Compare
            ...
            ElseIf x.Length <> 3 AndAlso y.Length <> 3 AndAlso x(6) > y(6) Then
                Return 1
            ElseIf x.Length <> 3 AndAlso y.Length <> 3 AndAlso x(6) < y(6) Then
                Return -1
            ElseIf x.Length = 7 AndAlso x(6) <> 0 Then
                Return 1
            ElseIf y.Length = 7 AndAlso y(6) <> 0 Then
                Return -1
            End If
            Return 0
        End Function
    ...
    Public Shared Function GenerateDefaultStops(Str As String, IndexToVerse As Integer()()) As Integer()
        ...
        'Next
        For Count = 0 To IndexToVerse.Length - 1
            If Count <> IndexToVerse.Length - 1 AndAlso IndexToVerse(Count)(1) = 0 AndAlso IndexToVerse(Count + 1)(1) = 1 Then DefStops.Add(IndexToVerse(Count + 1)(3) - 1)
            If IndexToVerse(Count)(6) <> 0 AndAlso Str(IndexToVerse(Count)(3)) <> ArabicData.ArabicSmallHighLigatureSadWithLamWithAlefMaksura AndAlso (Str(IndexToVerse(Count)(3)) <> ArabicData.ArabicSmallHighThreeDots Or DotToggle) Then DefStops.Add(IndexToVerse(Count)(3))
            If IndexToVerse(Count)(6) <> 0 AndAlso Str(IndexToVerse(Count)(3)) = ArabicData.ArabicSmallHighThreeDots Then DotToggle = Not DotToggle
        Next
        DefStops.Add(Str.Length)
    ...
    Public Function DoGetRenderedQuranTextW4W(Text As String, IndexToVerse As Integer()(), MetaRules As Generic.List(Of Arabic.FullRuleMetadata), UnfilteredMetaRules As Generic.List(Of Arabic.FullRuleMetadata), DefStops As Integer(), W4WLines As String()(), SchemeType As ArabicData.TranslitScheme, Scheme As String, IsLTRW4WTranslation As Boolean(), W4WNum As Boolean, NoArabic As Boolean, Colorize As Boolean, NoRef As Boolean) As RenderArray.RenderText
        ...
        For Count As Integer = 0 To IndexToVerse.Length - 1
            If IndexToVerse(Count)(4) = 1 AndAlso
                    (Arb.IsStop(ArbData.FindLetterBySymbol(Text(IndexToVerse(Count)(3)))) Or Text(IndexToVerse(Count)(3)) = ArabicData.ArabicStartOfRubElHizb Or Text(IndexToVerse(Count)(3)) = ArabicData.ArabicPlaceOfSajdah) Then
                If Not NoArabic Then
                ...
                If Not NoArabic Then
                    If Colorize Then
                        ...
                        If SchemeType <> ArabicData.TranslitScheme.None Then
                            Texts.AddRange(TranslitColors(IndexToVerse(Count)(2) - IndexToVerse(If(IndexToVerse(0)(6) <> 0, 1, 0))(2) + If(Text(IndexToVerse(Count)(3)) = ArabicData.ArabicEndOfAyah, 1, 0)))
                        Else
                            Texts.Add(New RenderArray.RenderText(RenderArray.RenderDisplayClass.eTransliteration, String.Empty))
                        End If
                    Else
                        Texts.Add(New RenderArray.RenderText(RenderArray.RenderDisplayClass.eArabic, Text.Substring(IndexToVerse(Count)(3), IndexToVerse(Count)(4))))
                        Texts.Add(New RenderArray.RenderText(RenderArray.RenderDisplayClass.eTransliteration, If(SchemeType <> ArabicData.TranslitScheme.None, TranslitWords(IndexToVerse(Count)(2) - IndexToVerse(If(IndexToVerse(0)(6) <> 0, 1, 0))(2) + If(Text(IndexToVerse(Count)(3)) = ArabicData.ArabicEndOfAyah, 1, 0)), String.Empty)))
                    End If
    ...
    Public Function DoGetRenderedQuranText(QuranText As String, IndexToVerse As Integer()(), Lines As String()(), W4WLines As String()(), SchemeType As ArabicData.TranslitScheme, Scheme As String, TranslationLang As String(), IsLTRW4WTranslation As Boolean(), W4W As Boolean, W4WNum As Boolean, NoArabic As Boolean, Header As Boolean, Colorize As Boolean, Verses As Boolean, NoRef As Boolean) As RenderArray
        ...
        Dim UnfilteredMetaRules As Generic.List(Of Arabic.FullRuleMetadata) = Nothing
        ...
        First = Index
        If Not NoArabic Then
            DefStops = GenerateDefaultStops(QuranText, IndexToVerse)
            Dim Idx As Integer
            If Not _CacheMetarules Is Nothing Then
                Idx = Array.BinarySearch(_IndexToVerse, New Integer() {IndexToVerse(0)(0), IndexToVerse(0)(1), IndexToVerse(0)(2)}, New TanzilReader.QuranWordChapterVerseWordComparer(False))
                If Idx < 0 Then Idx = Idx Xor -1
            End If
        ...
        While Index <> IndexToVerse.Length - 1 - If(_CacheMetarules Is Nothing, 1, 0)
            ...
            Do
                ...
                End If
                Dim MetaRules As Generic.List(Of Arabic.FullRuleMetadata) = Nothing
    ...
    Public Function GetTranslationVerse(Lines As String(), ByVal Chapter As Integer, ByVal Verse As Integer) As String
        GetTranslationVerse = Lines(If(Verse = 0, 0, GetVerseNumber(Chapter, Verse) - 1))
    End Function
    Public Function GetW4WTranslationVerse(Lines As String(), ByVal Chapter As Integer, ByVal Verse As Integer, ByVal Word As Integer) As String
        Dim Words As String() = Lines(If(Verse = 0, 0, GetVerseNumber(Chapter, Verse) - 1)).Split("|"c)
...
Public Class HadithReader
    ...
    End Function
    Public Function GetBookEName(ByVal BookNode As CachedData.HadithDocInfo.Books.Book, CollectionIndex As Integer) As String
        'If BookNode Is Nothing Then
        '    Return String.Empty
        'Else
        Dim Str As String = _PortableMethods.LoadResourceString("IslamInfo_" + ChData.IslamData.Collections(CollectionIndex).FileName + "Book" + CStr(BookNode.Index))
    ...
    Public Function GetChapterIndex(ByVal Index As Integer, ByVal BookIndex As Integer, ByVal HadithIndex As Integer) As Integer
        Dim BookNode As CachedData.HadithDocInfo.Books.Book = GetBookByIndex(Index, BookIndex)
        For Each ChapterNode As CachedData.HadithDocInfo.Books.Book.Chapter In BookNode.AllChapters
            If (ChapterNode.StartHadith <= HadithIndex And
                ChapterNode.StartHadith + ChapterNode.Hadiths > HadithIndex) Then Return ChapterNode.Index 'actual index in xml for missing entries instead of readable one?
        Next
        Return -1
    End Function
    Public Function GetHadithCount(ByVal Index As Integer, ByVal BookIndex As Integer) As Integer
        Return GetBookByIndex(Index, BookIndex).Hadiths
    End Function
