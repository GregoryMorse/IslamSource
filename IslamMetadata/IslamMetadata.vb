...
Public Class Arabic
    ...
    Class RuleMetadata
        Sub New()
        End Sub
        Sub New(NewIndex As Integer, NewLength As SByte, ByVal NewType As IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs(), NewOrigOrder As Short)
            Index = NewIndex
            Length = NewLength
            Type = NewType
    ...
    Public Function ReplaceMetadata(ArabicString As String, MetadataRule As RuleMetadata, LearningMode As Boolean) As String
        ...
        For Count As Integer = 0 To ChData.IslamData.ColorRuleSets(0).ColorRules.Length - 1
            ...
            Dim Match As String = Linq.Enumerable.FirstOrDefault(ChData.IslamData.ColorRuleSets(0).ColorRules(Count).Match, Function(Str As String)
                                                                                                                                TypeIndex = Linq.Enumerable.TakeWhile(MetadataRule.Type, Function(RuleWithArg As IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs) RuleWithArg.RuleName <> IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(Str)).Count
                    ...
                    Else
                        ...
                        For Index As Integer = 0 To Args.Length - 1
                            If Not Args(Index) Is Nothing And (LearningMode Or ChData.IslamData.ColorRuleSets(0).ColorRules(Count).MetaRuleFunc <> MetaRuleFuncs.eLearningMode Or Index <> 0) Then
                                Str.Append(ReplaceMetadata(Args(Index), New RuleMetadata(0, CSByte(Args(Index).Length), Linq.Enumerable.Select(MetadataRule.Type(TypeIndex).Args(Index), Function(S) New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs With {.RuleName = S}).ToArray(), CShort(Index)), LearningMode))
                            End If
                        Next
                    End If
                End If
    ...
    Public Function GetCacheMetarules(Lines As String(), IndexToVerse As Integer()()) As Generic.List(Of RuleMetadata)
        ...
        For Count As Integer = 0 To CacheIn.Length - 1
            ...
            Dim Key As IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() = Linq.Enumerable.Select(KeyVal(0).Substring(0, Math.Min(If(FirstBrace = -1, KeyVal(0).Length, FirstBrace), If(FirstCurlyBrace = -1, KeyVal(0).Length, FirstCurlyBrace))).Split("|"c), Function(S)
                                                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                                                               Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(ItRegEx.Replace(S, String.Empty)), .Args = If(M.Success, If(String.IsNullOrEmpty(M.Groups(1).Value), Nothing, Linq.Enumerable.Select(M.Groups(1).Value.Split(New Char() {","c}), Function(InnerStr) Linq.Enumerable.Select(InnerStr.Split(" "c), Function(St) IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(St)).ToArray())).ToArray(), Nothing)}
            ...
            For VerseCount = 0 To Vals.Length - 1
                ...
                Dim Ch As RuleMetadata() = If(FirstBrace = -1, Nothing, Linq.Enumerable.Select(Childs, Function(Item)
                                                                                                           ...
                                                                                                           Dim CDep As RuleMetadata() = If(FirstLT = -1, Nothing, Linq.Enumerable.Select(KeyVal(0).Substring(KeyVal(0).IndexOf("<"c) + 1, KeyVal(0).IndexOf(">"c) - KeyVal(0).IndexOf("<"c) - 1).Split("^"c), Function(DItem)
                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                  Return New RuleMetadata(CInt(DPieces(1)) + CInt(DPieces(1)) + Vals(VerseCount)(3), CSByte(DPieces(2)), If(String.IsNullOrEmpty(DPieces(0)), Nothing, Linq.Enumerable.Select(DPieces(0).Split("+"c), Function(S)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          Dim M As System.Text.RegularExpressions.Match = ArgRegEx.Match(S)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(ItRegEx.Replace(S, String.Empty)), .Args = If(M.Success, If(String.IsNullOrEmpty(M.Groups(1).Value), Nothing, Linq.Enumerable.Select(M.Groups(1).Value.Split(New Char() {","c}), Function(InnerStr) Linq.Enumerable.Select(InnerStr.Split(" "c), Function(St) IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(St)).ToArray())).ToArray(), Nothing)}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      End Function).ToArray()), CShort(DPieces(3)))
                                                                                                                                                                                                                                                                                                              End Function).ToArray())

                                                                                                           Dim Pieces As String() = Item.Substring(0, If(FirstLT = -1, Item.Length, FirstLT)).Split(";"c)
                                                                                                           Return New RuleMetadata(CInt(Pieces(1)), CSByte(Pieces(2)), If(String.IsNullOrEmpty(Pieces(0)), Nothing, Linq.Enumerable.Select(Pieces(0).Split("+"c), Function(S)
                                                                                                                                                                                                                                                                      Dim M As System.Text.RegularExpressions.Match = ArgRegEx.Match(S)
                                                                                                                                                                                                                                                                      Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(ItRegEx.Replace(S, String.Empty)), .Args = If(M.Success, If(String.IsNullOrEmpty(M.Groups(1).Value), Nothing, Linq.Enumerable.Select(M.Groups(1).Value.Split(New Char() {","c}), Function(InnerStr) Linq.Enumerable.Select(InnerStr.Split(" "c), Function(St) IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(St)).ToArray())).ToArray(), Nothing)}
                                                                                                                                                                                                                                                                  End Function).ToArray()), CShort(Pieces(3))) With {.Dependencies = CDep}
                                                                                                       End Function).ToArray())
                Dim Dep As RuleMetadata() = If(FirstCurlyBrace = -1, Nothing, Linq.Enumerable.Select(Deps, Function(Item)
                                                                                                               ...
                                                                                                               Return New RuleMetadata(CInt(Pieces(1)) + IndexToVerse(Idx)(3) + Vals(VerseCount)(3), CSByte(Pieces(2)), If(String.IsNullOrEmpty(Pieces(0)), Nothing, Linq.Enumerable.Select(Pieces(0).Split("+"c), Function(S)
                                                                                                                                                                                                                                                                                                                       Dim M As System.Text.RegularExpressions.Match = ArgRegEx.Match(S)
                                                                                                                                                                                                                                                                                                                       Return New IslamData.RuleMetaSet.RuleMetadataTranslation.RuleWithArgs() With {.RuleName = IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(ItRegEx.Replace(S, String.Empty)), .Args = If(M.Success, If(String.IsNullOrEmpty(M.Groups(1).Value), Nothing, Linq.Enumerable.Select(M.Groups(1).Value.Split(New Char() {","c}), Function(InnerStr) Linq.Enumerable.Select(InnerStr.Split(" "c), Function(St) IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(St)).ToArray())).ToArray(), Nothing)}
                                                                                                                                                                                                                                                                                                                   End Function).ToArray()), CShort(Pieces(3)))
                                                                                                           End Function).ToArray())
                Rules.Add(New RuleMetadata(IndexToVerse(Idx)(3) + Vals(VerseCount)(3), CSByte(Vals(VerseCount)(4)), Key, CShort(Integer.Parse(KeyVal(2)))) With {.Children = Ch, .Dependencies = Dep})
    ...
    Public Shared Function GetMetarulesFromCache(ArabicString As String, BasePosition As Integer, Rules As Generic.List(Of RuleMetadata)) As Generic.List(Of RuleMetadata)
        ...
        While Index <> Rules.Count - 1 AndAlso Rules(Index).Index = Rules(Index + 1).Index
            Index += 1
        End While
        Return FilterMetadataStopsContig(ArabicString.Length + BasePosition, Linq.Enumerable.Reverse(Linq.Enumerable.TakeWhile(Linq.Enumerable.Skip(Linq.Enumerable.Reverse(Rules).ToList(), Rules.Count - Index - 1).ToList(), Function(Item) Item.Index < BasePosition + ArabicString.Length).ToList()).ToList(), Nothing, BasePosition)
    End Function
                ...
                Get
                    If _SplitEvaluator Is Nothing And Not _Evaluator Is Nothing Then
                        _SplitEvaluator = Linq.Enumerable.Select(_Evaluator.Split(";"c), Function(Str) Linq.Enumerable.Select(Str.Split("|"c), Function(S)
                                                                                                                                                   ...
                                                                                                                                                   Return New RuleWithArgs With {.RuleName = GetStringIndex(Arabic.ItRegEx.Replace(S, String.Empty)), .Args = If(M.Success, Linq.Enumerable.Select(M.Groups(1).Value.Split(","c), Function(InnerStr) Linq.Enumerable.Select(InnerStr.Split(" "c), Function(St) GetStringIndex(St)).ToArray()).ToArray(), Nothing)}
    ...
    Public Sub CheckMutualExclusiveRules(bAssumeContinue As Boolean, VerIndex As Integer)
        ...
        For MainCount = 0 To ChData.IslamData.VerificationSet(VerIndex).MetaRules.Length - 1
            ...
            For Count = 0 To Matches.Count - 1
                ...
                If Not bSieve Then
                    For SubCount = 0 To MetaRules.Length - 1
                        If Array.IndexOf(Linq.Enumerable.Select(MatchMetadata, Function(MM) IslamData.RuleMetaSet.RuleMetadataTranslation.GetStringIndex(MM)).ToArray(), MetaRules(SubCount)(0).RuleName) <> -1 And Matches(Count).Groups(SubCount + 1).Success Then
